...
import { getFirestore, doc, setDoc, getDoc, addDoc, collection, query, orderBy, limit, getDocs, writeBatch } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
const translations = {
    en: {
        ...
        homeHistory: "Quiz History", shareCardTitle: "Share Your Score", downloadBtn: "Save", shareBtn: "Share", tabAllTime: "All Time", tabThisWeek: "This Week", leaderboardWeeklyReset: "WEEKLY RESET", leaderboardResetsIn: "Resets in", leaderboardYou: "You", leaderboardSortXP: "XP", leaderboardSortStreak: "Streak", profileTotalQuizzes: "Total Quizzes", profileAccuracy: "Accuracy", achievementSheetTitle: "Achievements", homeQuickExplore: "Quick Explore",
...
function relocateSettingsPanel() {
    ...
    else if (!isDesktop && settingsPanel.parentElement !== mobileHost) mobileHost.appendChild(settingsPanel);
}
...
function renderLeaderboardPodium(topThree, tab) {
    ...
    topThree.forEach((d, i) => {
        ...
        const ringClass = isMe ? 'ring-[var(--primary-color)]' : t.ring;
        const glowClass = isMe ? 'shadow-[0_0_15px_rgba(var(--primary-color-rgb),0.6)]' : t.glow;
        html += `
...
function setupEventListeners() {
    ...
    if (accuracyStat) {
        const lifetimeCorrect = parseInt(localStorage.getItem('flagx-lifetimecorrect') || 0);
        const lifetimeAttempted = parseInt(localStorage.getItem('flagx-lifetimeattempted') || 0);
        accuracyStat.textContent = lifetimeAttempted > 0 ? Math.round((lifetimeCorrect / lifetimeAttempted) * 100) + '%' : '–';
    }
